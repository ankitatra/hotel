...
const MainMenu = ({ style = "" }) => {
  ...
  const handleLinkClick = (e) => {
    // if (!localStorage.getItem('token')&&localStorage.getItem('accountType')=="vendor") {
    //   e.preventDefault();
    //   toast.error('Please login first');
    // }
    if (localStorage.getItem('accountType') !== "vendor") {
      e.preventDefault();
      toast.error('You need to be a expert to access this page');
    }
        ...
        >
          ...
          <ul className="subnav ">
            {dashboardItems.map((menu, i) => (
              ...
              >
                <Link to={menu.routePath} onClick={handleLinkClick}>{menu.name}</Link>
              </li>
            ))}
          </ul>
        </li>
